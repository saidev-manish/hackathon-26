"""Fix all files corrupted by the r->o PowerShell encoding bug."""
import pathlib, sys

BASE = pathlib.Path(r'c:\Users\saide\Desktop\hackathon26\mobile')

# ─── period-dashboard.js ───────────────────────────────────────────────────────
PERIOD_DASHBOARD = r"""import React, { useEffect, useMemo, useRef, useState } from 'react';
...
function getSafeCycleLength(year, januaryStartDay, februaryStartDay) {
    ...
    if (rawCycleLength < 21 || rawCycleLength > 35) return 28;
...
export default function PeriodDashboardScreen() {
    ...
    useEffect(() => {
        ...
        const enforcePeriodRoute = async () => {
            ...
            if (!active) return;
            if (careType === 'pcos') { router.replace('/pcos-empty'); return; }
            if (careType !== 'period') { router.replace('/care-type'); }
        };
        enforcePeriodRoute().catch((error) => console.error('Failed to enforce period dashboard route', error));
        return () => { active = false; };
    }, [currentUser?.uid, router]);

    const monthsData = useMemo(
        () => MONTHS.map((monthName, monthIndex) => ({ monthName, rows: getMonthRows(year, monthIndex) })),
        [year]
    );
    ...
    const prediction = useMemo(() => {
        ...
        for (let monthIndex = 2; monthIndex <= 11; monthIndex += 1) predictedByMonth[monthIndex] = [];
        ...
        if (firstPredictedStartDate.getFullYear() !== year) return { predictedByMonth, hasPrediction: false };
        ...
        <SafeAreaView style={styles.container}>
            ...
            <View style={styles.content}>
                ...
                <Text style={styles.hintText}>
                    Select all period dates in January and February. Predicted dates are highlighted from March onwards.
                </Text>
                <Text style={styles.limitText}>Maximum {MAX_SELECTED_DATES} dates can be selected per month.</Text>
                {selectionWarning ? <Text style={styles.warningText}>{selectionWarning}</Text> : null}
                {prediction.hasPrediction ? (
                    <Text style={styles.predictionText}>Predicted period dates are highlighted for upcoming months.</Text>
                ) : null}
...
const styles = StyleSheet.create({
    ...
    hamburger: { width: 40, height: 40, borderRadius: 12, backgroundColor: theme.colors.surface, borderWidth: 1, borderColor: theme.colors.inputBorder, alignItems: 'center', justifyContent: 'center' },
    ...
    drawer: { width: 270, height: '100%', backgroundColor: theme.colors.surface, paddingTop: 56, paddingHorizontal: 20, paddingBottom: 30 },
    ...
    calendarCard: { backgroundColor: theme.colors.surface, borderRadius: 16, paddingVertical: 14, paddingHorizontal: 10, borderWidth: 1, borderColor: theme.colors.inputBorder },
    ...
    arrowButton: { width: 44, height: 44, borderRadius: 22, backgroundColor: theme.colors.surface, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: theme.colors.inputBorder },
...
# ─── Write files ────────────────────────────────────────────────────────────────
files = {
    BASE / 'app' / '(period)' / 'period-dashboard.js': PERIOD_DASHBOARD,
}
...
print("Done writing period-dashboard.js")
